...
class EMACallback(Callback):
    def __init__(self, len_updates, ema_ckpt_path=None) -> None:
        super().__init__()
    ...
    def on_fit_start(self, trainer, pl_module):
        ...
        # fcj: load ema ckpt
        if self.ema_ckpt_path:
            ckpt_model = torch.load(self.ema_ckpt_path, map_location='cpu')
            state_dict = ckpt_model['state_dict']
            for k, v in trainer.ema_model.ema.state_dict().items():
                v += (state_dict['model.' + k].type_as(v) - v)
            global_step = ckpt_model['global_step']
            trainer.ema_model.updates += global_step
            print('finish loading ema ckpt!')
    ...
    def on_train_epoch_end(self, trainer, pl_module) -> None:
        ...
        torch.save(
            checkpoint,
            os.path.join(trainer.log_dir, f'ema_{trainer.current_epoch}.pth'))
        # save original ckpt
        trainer.save_checkpoint(
            os.path.join(trainer.log_dir, f'origin_{trainer.current_epoch}.pth'))
